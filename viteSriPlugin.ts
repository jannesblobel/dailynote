...
export function sriPlugin(): Plugin {
  ...
  const bundle: Record<string, { fileName: string; type: string; source?: string; code?: string }> = {};
  ...
  return {
    ...
    async writeBundle(options, outputBundle) {
      ...
      await Promise.all(htmls.map(async ({ name, source: html }) => {
        const $ = load(String(html));
        const scripts = $('script').filter('[src]');
        const stylesheets = $('link[rel="stylesheet"]').filter('[href]');

        const calculateIntegrity = async (element: { attribs: Record<string, string | undefined> }) => {
          ...
          const asset = Object.values(bundle).find((item) => item.fileName === resourcePath);
          ...
          if (asset) {
            ...
            try {
              source = readFileSync(resolve(options.dir ?? config.build.outDir, resourcePath));
            } catch {
              source = undefined;
            }
          }
