...
import { base64ToBytes, bytesToBase64, encodeUtf8, decodeUtf8, randomBytes } from './cryptoUtils';
import {
  NOTES_STORE,
  NOTE_META_STORE,
  openUnifiedDb,
...
export async function decryptNoteContent(
  ...
  const parsed = JSON.parse(decodeUtf8(new Uint8Array(decrypted))) as { content: string };
...
export async function getNoteMeta(date: string): Promise<NoteMetaRecord | null> {
  const db = await openUnifiedDb();
  return new Promise((resolve, reject) => {
...
export async function setNoteAndMeta(record: NoteRecord, meta: NoteMetaRecord): Promise<void> {
  const db = await openUnifiedDb();
  return new Promise((resolve, reject) => {
